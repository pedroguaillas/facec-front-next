...
const handleApiCall = async (
    ...
    try {
        const response = await axiosAuth.get(`retentions/${endpoint}/${id}`);
...
export const Dropdown = ({ isOpen, index, shop, only, setIsOpen }: Props) => {
    ...
    const getOptions = () => {
        const options = [
            { label: "Ver Pdf", onClick: viewRetentionPdf },
    ...
    const viewRetentionPdf = async () => {
        try {
            const response = await axiosAuth.get(`retentions/pdf/${shop.id}`, { responseType: 'blob' });
    ...
    const downloadXml = async () => {
        try {
            const response = await axiosAuth.get('retentions/download/' + shop.id);
            if (response.status >= 200) {
                const a = document.createElement('a') //Create <a>
                a.href = 'data:text/xml;base64,' + response.data.xml //Image Base64 Goes here
                a.download = `Retención ${shop.atts.serie_retencion}.xml` //File name Here
